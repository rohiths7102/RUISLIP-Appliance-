 ...
 * real products (thumbnail · name · price, or "Call for price" — never a number
 * the owner withholds). Enter with nothing highlighted, or the "See all"
 * footer, lands on /products?q=… — the same entry point the sitelinks
 * SearchAction schema advertises to Google.
 */
export default function SearchBar({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    const q = v.trim();
    if (q.length < 2) { setHits([]); setTotal(0); setOpen(false); return; }
    const t = setTimeout(async () => {
      ...
      try {
        ...
        setHits(j.items || []); setTotal(j.total || 0); setOpen(true); setActive(-1);
  ...
  const submit = () => {
    const q = v.trim();
    if (active >= 0 && hits[active]) return go(`/products/${hits[active].slug}`);
    ...
    <div ref={boxRef} className={`relative ${className}`}>
      ...
      >
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "ArrowDown") { e.preventDefault(); if (hits.length) { setOpen(true); setActive((a) => (a + 1) % hits.length); } }
            else if (e.key === "ArrowUp") { e.preventDefault(); if (hits.length) { setOpen(true); setActive((a) => (a <= 0 ? hits.length - 1 : a - 1)); } }
