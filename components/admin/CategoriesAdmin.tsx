...
export default function CategoriesAdmin({ initial }: { initial: Row[] }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between"><h1 className="font-display text-2xl font-semibold">Categories <span className="text-sm text-ink/40">({rows.length})</span></h1>{msg && <span className="text-sm text-blue">{msg}</span>}</div>
      <div className="mt-4 overflow-x-auto rounded-2xl border border-line bg-white"><table className="w-full text-sm">
        <thead className="bg-paper-2 text-left text-ink/60"><tr><th className="p-3">Name</th><th className="p-3">Products</th><th className="p-3">Visible</th><th className="p-3">Order</th><th className="p-3"></th></tr></thead>
        <tbody>{rows.map((r) => (
          <tr key={r.id} className="border-t border-line"><td className="p-3">{r.name}{r.parentId ? <span className="ml-1 text-xs text-ink/40">(sub)</span> : null}</td><td className="p-3">{r.productCount}</td><td className="p-3">{r.isVisible ? "Yes" : "No"}</td><td className="p-3">{r.order}</td>
            <td className="p-3"><button onClick={() => { setSel(r); setMsg(""); }} className="rounded-full border border-navy/20 px-3 py-1 text-xs hover:border-blue">Edit</button></td></tr>
        ))}</tbody>
      </table></div>
      {sel && (
