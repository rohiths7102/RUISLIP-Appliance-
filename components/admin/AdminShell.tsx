      ...
      <div className="flex">
        <aside className="hidden w-56 shrink-0 border-r border-line bg-white md:block">
          ...
          <nav className="flex flex-col px-2 pb-4 text-sm">
            {NAV.map(([sub, label]) => (
              <Link key={sub} href={adminHref(sub)} className={`rounded-lg px-3 py-2 ${activeSub === sub ? "bg-navy text-paper" : "text-ink/70 hover:bg-paper-2"}`}>{label}</Link>
            ))}
          </nav>
        </aside>
        <div className="flex-1">
          <header className="flex items-center justify-between border-b border-line bg-white px-6 py-3">
            <Link href="/" className="text-sm text-ink/60 hover:text-blue">← View site</Link>
            <div className="flex items-center gap-3 text-sm"><span className="text-ink/60">{email}</span><LogoutButton signinPath={SIGNIN_PATH} /></div>
          </header>
          ...
          {usingDevPassword() && (
            <div className="border-b-2 border-red-300 bg-red-50 px-6 py-3 text-sm text-red-900">
              ...
              <p className="mt-1 text-[13px] leading-relaxed">
                ...
                <code className="rounded bg-red-100 px-1.5 py-0.5 font-mono text-xs">node scripts/hash-password.mjs &quot;your-password&quot;</code>,
