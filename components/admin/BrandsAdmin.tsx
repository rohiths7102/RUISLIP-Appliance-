...
type Row = { id: string; name: string; slug: string; logo: string; description: string; productCount: number; isVisible: boolean; order?: number; priceOnApplication?: boolean };
export default function BrandsAdmin({ initial }: { initial: Row[] }) {
  ...
  async function save() {
    ...
    const r = await fetch(`/api/admin/brands/${sel.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ logo: sel.logo, description: sel.description, isVisible: sel.isVisible, order: rank, priceOnApplication: !!sel.priceOnApplication }) });
    setSaving(false);
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {rows.map((r) => (
          <Card key={r.id} className="p-4 text-center">
            ...
            <div className="mt-2 text-sm font-medium">{r.name}</div><div className="text-xs text-muted">{r.productCount} products · {r.isVisible ? "visible" : "hidden"}{r.priceOnApplication ? " · call for price" : ""}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setSel(null)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 grid gap-3 text-sm">
              ...
              <label className="flex items-start gap-2"><input type="checkbox" checked={!!sel.priceOnApplication} onChange={(e) => upd("priceOnApplication", e.target.checked)} className="mt-0.5" /> <span>Call for price <span className="text-xs text-muted">— hides the price on every product of this brand, everywhere (site, search, chatbot, Google feed)</span></span></label>
