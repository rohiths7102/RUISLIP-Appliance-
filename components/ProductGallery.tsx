    ...
    <div className="lg:sticky lg:top-[96px]">
      <div className="relative aspect-square overflow-hidden rounded-[4px] border border-ink/10 bg-gradient-to-br from-white to-paper-2">
        {current ? (
          ...
          <img src={current} alt={title} className="shot h-full w-full object-contain p-14" />
        ) : (
        ...
        <div className="absolute left-4 top-4 rounded-sm bg-navy/85 px-3 py-1.5 backdrop-blur-sm">
      ...
      {shots.length > 1 && (
        <div className="mt-3 flex flex-wrap gap-2.5">
          {shots.slice(0, 8).map((g, i) => (
            ...
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={g} alt="" loading="lazy" className="shot h-full w-full object-contain" />
            </button>
          ))}
        </div>
      )}
