...
async function rows() {
  try { const db = await getPrisma(); const rs = await db.brand.findMany({ orderBy: { name: "asc" }, select: { id: true, name: true, slug: true, logo: true, description: true, productCount: true, isVisible: true } }); if (rs.length) return rs; } catch {}
